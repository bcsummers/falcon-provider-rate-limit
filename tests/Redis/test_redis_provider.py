...
from falcon_provider_redis import redis_client
...
rc = redis_client(host=REDIS_HOST, port=REDIS_PORT)
rc.delete('dos-limit')
rc.delete('rate-limit')
rc.delete('no-auth-key/middleware')
rc.delete('127.0.0.1')
rc.delete('127.0.0.1/middleware')
...
def test_authenticated_rate_limit(
    ...
    * validate rate-limit timeout expires and request return to normal.
    ...
    Args:
        client_redis_1 (fixture): Pytest fixture for falcon.testing.Testclient.
    ...
    monkeypatch.setitem(RedisResource.rate_limit_control, 'authenticated_limit', int(limit))
    ...
    # make request
    response: Result = client_redis_1.simulate_get('/middleware', params={'key': param})
...
def test_unauthenticated_rate_limit(
    ...
    * validate proper headers are set including retry-after.
    ...
    Args:
        client_redis_2 (fixture): Pytest fixture for falcon.testing.Testclient.
        param (str): A query param value to send to request.
        limit (str): The expected rate limit value for assert (set in app.py).
        remaining (str): The expected number of remaining rate limit request for assert.
        status_code (int): The expected status code for assert.
        ...
        monkeypatch (fixture): Pytest _pytest.monkeypatch.MonkeyPatch object.
    """
    monkeypatch.setitem(RedisResource.rate_limit_control, 'unauthenticated_limit', int(limit))
    monkeypatch.setitem(RedisResource.rate_limit_control, 'global_limit', False)

    # make request
    response: Result = client_redis_2.simulate_get('/middleware', params={'key': param})
...
def test_unauthenticated_with_none_auth_keys(
    ...
    * validate proper headers are set including retry-after.
    ...
    Args:
        client_redis_3 (fixture): Pytest fixture for falcon.testing.Testclient.
        param (str): A query param value to send to request.
        limit (str): The expected rate limit value for assert (set in app.py).
        remaining (str): The expected number of remaining rate limit request for assert.
        status_code (int): The expected status code for assert.
        ...
        monkeypatch (fixture): Pytest _pytest.monkeypatch.MonkeyPatch object.
    """
    monkeypatch.setattr(RateLimitMiddleware, 'client_key', 'no-client-key', raising=False)
    monkeypatch.setitem(RedisResource.rate_limit_control, 'unauthenticated_limit', int(limit))
    monkeypatch.setitem(RedisResource.rate_limit_control, 'global_limit', False)

    # make request
    response: Result = client_redis_3.simulate_get('/middleware', params={'key': param})
...
def test_dos_limit(
    ...
    * validate proper headers are set including retry-after.
    ...
    Args:
        ...
        monkeypatch (fixture): Pytest _pytest.monkeypatch.MonkeyPatch object.
    """
    monkeypatch.setitem(RedisResource.rate_limit_control, 'dos_limit', int(limit))
    monkeypatch.setitem(RedisResource.rate_limit_control, 'global_limit', False)
...
def test_disabled(
    client_redis_1: object, param: str, status_code: int, text: str, monkeypatch: object,
):
    ...
    * disable rate limit at the resource level.
    ...
    Args:
        ...
        monkeypatch (fixture): Pytest _pytest.monkeypatch.MonkeyPatch object.
    """
    monkeypatch.setitem(RedisResource.rate_limit_control, 'enabled', False)
...
@pytest.mark.parametrize('param,status_code,text', [('first', 200, 'first-worked')])
def test_no_limits(client_redis_3: object, param: str, status_code: int, text: str):
    """Testing disabled rate limit
